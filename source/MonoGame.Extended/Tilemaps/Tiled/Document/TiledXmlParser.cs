...
namespace MonoGame.Extended.Tilemaps.Tiled.Document
{
    internal static class TiledXmlParser
    {
        public static TiledMapXml ParseMap(XElement element)
        {
            ...
            foreach (var child in element.Elements())
            {
                switch (child.Name.LocalName)
                {
                    ...
                    case "tileset":
                    {
                        map.Tilesets.Add(ParseTilesetRef(child));
                        break;
                    }
                    case "layer":
                    {
                    ...
                    default:
                    {
                        Trace.TraceWarning($"Ignoring element: {child.Name.LocalName}");
                        break;
                    }
                }
            }
        ...
        public static TiledTilesetXml ParseTileset(XElement element)
        {
            ...
            ParseBaseTileset(element, tileset);
            
            return tileset;
        }

        private static TiledTilesetRefXml ParseTilesetRef(XElement element)
        {
            var tilesetReference = new TiledTilesetRefXml
            {
                Source = (string)element.Attribute("source"),
            };
            ParseBaseTileset(element, tilesetReference);
            
            return tilesetReference;
        }

        private static void ParseBaseTileset(XElement element, TiledTilesetXml tileset)
        {
            ...
            foreach (var child in element.Elements())
            {
                switch (child.Name.LocalName)
                {
                    ...
                    default:
                    {
                        Trace.TraceWarning($"Ignoring child of base tileset: {child.Name.LocalName}");
                        break;
                    }
                }
            }
        ...
        private static TiledTileXml ParseTile(XElement element)
        {
            ...
            foreach (var child in element.Elements())
            {
                switch (child.Name.LocalName)
                {
                    ...
                    default:
                    {
                        Trace.TraceWarning($"Ignoring child of tile: {child.Name.LocalName}");
                        break;
                    }
                }
            }
        ...
        private static TiledObjectXml ParseObject(XElement element)
        {
            ...
            foreach (var child in element.Elements())
            {
                switch (child.Name.LocalName)
                {
                    ...
                    default:
                    {
                        Trace.TraceWarning($"Ignoring child of object: {child.Name.LocalName}");
                        break;
                    }
                }
            }
        ...
        private static TiledGroupLayerXml ParseGroupLayer(XElement element)
        {
            ...
            foreach (var child in element.Elements())
            {
                switch (child.Name.LocalName)
                {
                    ...
                    default:
                    {
                        Trace.TraceWarning($"Ignoring child of group: {child.Name.LocalName}");
                        break;
                    }
                }
            }
        ...
        private static int ParseInt(string val, int defaultValue = 0)
        {
            if (string.IsNullOrWhiteSpace(val))
            {
                return defaultValue;
        ...
        private static uint ParseUInt(string val, uint defaultValue = 0u)
        {
            if (string.IsNullOrWhiteSpace(val))
            {
                return defaultValue;
            ...
            return uint.TryParse(val, NumberStyles.Integer, CultureInfo.InvariantCulture, out uint result)
                ? result
                : defaultValue;
        }
