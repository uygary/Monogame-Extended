...
namespace MonoGame.Extended.Graphics.Effects
{
    ...
    public class EffectResource
    {
        ...
        private static string DetermineShaderExtension(GraphicsDevice graphicsDevice)
        {
            ...
            // Return the cached result if we've already figured it out.
            if (_detectedShaderProfile != null)
            {
                return _detectedShaderProfile;
            }
            
            // Perform a Bytecode Capability Test.
            string[] profilesToTest = ["dx12", "dx11", "ogl", "vk"];
            foreach (string profile in profilesToTest)
            {
                try
                {
                    Trace.TraceInformation($"Testing shader profile: {profile}");
                    ...
                    _detectedShaderProfile = profile;
                    
                    return _detectedShaderProfile;
                }
                catch
                {
                    // Bytecode was rejected by the current graphics backend: Try the next possibility.
                    Trace.TraceWarning($"Shader profile was rejected: {profile}");
